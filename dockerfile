...
COPY . .
RUN git clone https://github.com/Chrcou/convertion1080p.git
WORKDIR /app/convertion1080p

RUN npm i
RUN echo 'inputPath="/toConvert/downloads/"' >.env
RUN echo 'outputPath="/toConvert/1080p/"' >>.env
RUN ls
RUN npm run build
RUN ls
